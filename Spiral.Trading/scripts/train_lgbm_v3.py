#!/usr/bin/env python3
"""LightGBM v3 with bar-relative normalization."""
...
def load_features(path: str, max_rows: int = None):
    """Load data using TradingDatasetV3 and flatten features."""
    print(f"Loading {path}...")
    ds = TradingDatasetV3(path, window_size=60, stride=5)
    ...
    for i, idx in enumerate(indices):
        ...
        # Flatten 1s features (60, 3) -> (180,)
        f_1s = sample['features_1s'].numpy().flatten()
        
        # Summary stats for 1m (60, 3) - 3 channels now
        ...
        features = np.concatenate([f_1s, f_1m_stats, f_5m_stats])  # (204,)
...
def main():
    ...
    model.save_model('data/lgbm_v3_model.txt')
    print("Saved model to data/lgbm_v3_model.txt")
